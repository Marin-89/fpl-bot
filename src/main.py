...
Design: sends a fresh Telegram message every time the lineup or stage
changes, always visible at the bottom of the chat — no edit-in-place,
since that silently updated old messages buried in chat history rather
than appearing where the user could actually find them.
...
otherwise pick. Captain/vice overrides work the same way. Both persist
...
def run_build_squad(budget: float):
    ...
    state["last_sent_lineup_text"] = None
...
def _run_daily_inner(state: dict):
    ...
    # --- Always send fresh (no edit-in-place) — but stay quiet if this exact
    # gameweek/stage/text combination was already sent, so we don't spam
    # identical messages every 30 minutes when nothing has actually changed. ---
    last_sent = state.get("last_sent_lineup_signature", {})
    signature = {"gameweek": target_event["id"], "stage": stage, "text": message_text}
    already_sent_this_exact_state = last_sent == signature

    if not already_sent_this_exact_state:
        telegram_bot.send_message(message_text)
        state["last_sent_lineup_signature"] = signature
        print(f"Sent {stage_display} for GW{target_event['id']}")
    else:
        print("No change since last check — staying silent.")

    entering_final_window_now = stage == "final_window" and state.get("_last_stage_seen") != "final_window"
    should_alert = material_change or entering_final_window_now or (stage == "late_check_window" and changes)
    state["_last_stage_seen"] = stage

    if should_alert and already_sent_this_exact_state:
        # Edge case: stage just changed but text happens to match a prior
        # send (rare) — still worth a nudge that we've entered a new stage.
        pass
    elif should_alert:
        alert_reason = (
        ...
        telegram_bot.send_message(f"🔔 *{alert_reason}* — see the lineup above.")
